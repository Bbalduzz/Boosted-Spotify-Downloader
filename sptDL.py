...
def download_track(track, playlist_name, link):
	track_name = track['name']
	track_arists = track['artist']
...
def process_starter(track_id, playlist_name):
	track = get_tracks_data(track_id)
	link = get_links(track)
	download_track(track, playlist_name, link)
...
if __name__ == '__main__':
	try:
		...
		jobs = []
		for i in track_ids:
			downloader = multiprocessing.Process(target=process_starter, args=(i,playlist_name))
			jobs.append(downloader)
			downloader.start()
